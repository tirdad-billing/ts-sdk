...
async function run() {
  const result = await tirdad.environments.cloneEnvironment("<id>", {});
...
import { environmentsCloneEnvironment } from "@tirdad-ai/sdk/funcs/environmentsCloneEnvironment.js";
...
async function run() {
  const res = await environmentsCloneEnvironment(tirdad, "<id>", {});
  if (res.ok) {
