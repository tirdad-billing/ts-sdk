...
<!-- UsageSnippet language="typescript" operationID="getCouponByCode" method="get" path="/coupons/code/{code}" -->
```typescript
import { Tirdad } from "@tirdad-ai/sdk";
...
async function run() {
  const result = await tirdad.coupons.getCouponByCode("<value>");
...
```typescript
import { TirdadCore } from "@tirdad-ai/sdk/core.js";
import { couponsGetCouponByCode } from "@tirdad-ai/sdk/funcs/coupons-get-coupon-by-code.js";
...
async function run() {
  const res = await couponsGetCouponByCode(tirdad, "<value>");
  if (res.ok) {
    const { value: result } = res;
    console.log(result);
  } else {
    console.log("couponsGetCouponByCode failed:", res.error);
  }
}
