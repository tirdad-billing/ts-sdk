 ...
 */
export function webhookEventsPostWebhookEventsCheckoutSessionCompleted(
  client: TirdadCore,
  options?: RequestOptions,
): APIPromise<
  Result<
    models.WebhookDtoCheckoutSessionWebhookPayload,
    | TirdadError
...
> {
  const path = pathToFunc("/webhook-events/checkout.session.completed")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, models.WebhookDtoCheckoutSessionWebhookPayload$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
